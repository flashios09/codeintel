...
class PHPFunction:
    def __init__(self, funcname, phpArgs, lineno, depth=0,
                 ...
                 returnType=None, returnByRef=False, currentNamespace=None):
        self.name = funcname
        self.args = phpArgs
...
#            attributes += ['__dtor__']
        ...
        self.signature = "%s::<var class='function_name'>%s</var>" % (self.currentNamespace.name, self.signature, )
        self.signature += "("
        if self.args:
            self.signature +=  ", ".join([x.signature for x in self.args])
        self.signature += ")"

    # def parsePHPDocBlockO(self, docblock):
        ...
        try:
            for docstr in docstrings:
                ...
                if docstr.startswith("@"):
                    docstr = docstr.split()
                    docblock_parsed += "<var class='at'>%s</var> %s\n" % (docstr[0], " ".join(docstr[1:]))

                # comments which have not to be parsed (skip empty strings)
                # elif len(docstr.strip()) > 0:
                # comments which have not to be parsed (don't skip empty strings)
                else:
                    docblock_parsed += "%s\n" % docstr
